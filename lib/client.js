 ...
 *
 *  - `conversation.chat.assistant-actions` — a per-message read button with
 *    play / pause / resume / stop states.
 *  - `conversation.session.header.actions` — a "read the whole conversation"
 *    button, also with pause / resume / stop states.
 *  - `settings.plugin.item` — the configuration card (backend picker, rate,
 *    voice), persisted to localStorage. Keyed by the `dsh-read-aloud` settings
 *    namespace; the host half registers that namespace so the Plugins tab
 *    serves the key and dispatches this card.
 *
 * Both playback paths support pause/resume: the browser path via
 * speechSynthesis.pause()/resume(), the host path via <audio> pause()/play().
 *
...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// ── long-text chunking ──────────────────────────────────────────────────
		/**
		 * Cloud TTS backends cap a single synthesis request (OpenAI tts-1 ≈ 4096
		 * characters, ElevenLabs ≈ 5000, Azure SSML similar), and the browser
		 * engine also chokes on very long utterances. A message is therefore
		 * split at sentence boundaries into chunks of at most MAX_TTS_CHARS,
		 * each synthesized and played in turn by the existing queue.
		 */
		const MAX_TTS_CHARS = 4000;

		/** True when text[index] ends a sentence (CJK/Latin enders; `.` only before whitespace). */
		...
		/**
		 * Split text into chunks of at most `maxChars`, cutting at the last
		 * sentence boundary inside each window, then the last space, then hard.
		 * @returns the text as-is when it fits, else a list of chunk strings.
		 */
		function chunkText(text, maxChars) {
			...
			while (start < src.length) {
				...
				if (end < src.length) {
					...
					if (cut <= start) {
						const space = src.lastIndexOf(" ", end - 1);
						cut = space > start ? space : end;
					}
					end = cut;
				}
		...
		function createSpeechController({ connection }) {
			...
			let queue = []; // [{ key, text }]
			let epoch = 0;
			let paused = false; // pause state of the current playback
			let pausedGap = false; // advance() bailed mid-pause with queued items
			let resumeWaiters = []; // resolve()s for advanceHost waiting out a pause
			...
			/** Resolve every waiter parked by waitWhilePaused(). */
			function flushResumeWaiters() {
			...
			function cancel() {
				...
				if (browserSupported) {
					try {
						...
						// Chrome leaves the engine "paused" after cancel(); the
						// next speak() silently drops without this reset.
			...
			/** Pause the current playback (browser utterance or host <audio>). */
			function pause() {
			...
			/** Resume the current playback; also un-parks a synthesis awaiting pause. */
			function resume() {
				if (!paused) return;
				paused = false;
				if (browserSupported) {
				...
				if (audioEl) {
					try { audioEl.play().catch(() => {}); } catch { /* ignore */ }
				}
				const kick = pausedGap;
				pausedGap = false;
				flushResumeWaiters();
				notify();
				if (kick) advance();
			...
			/** Resolve once unpaused; used by host synthesis before starting audio. */
			...
			function advanceBrowser() {
				...
				if (paused) {
					// A chunk finished while paused: hold the queue until resume().
					if (queue.length === 0) {
						endQueue();
						return;
					}
			...
			async function advanceHost() {
				...
				// If the user paused while the request was in flight, hold the
				// audio until they resume (or cancel).
			...
			/** Expand one message into queue entries (one per ≤ MAX_TTS_CHARS chunk, same key). */
			function enqueue(key, text) {
				return chunkText(text, MAX_TTS_CHARS).map((chunk) => ({ key, text: chunk }));
			}
			...
			function speakAll(items) {
				...
				for (const item of items) {
					queue.push(...enqueue(item.key, item.text));
				}
				advance();
			}
			...
			return {
				browserSupported,
				subscribe, isSpeaking, isReading, isPaused: () => paused,
				getSettings, getVoices, getError,
				setSetting, speak, speakAll, cancel, pause, resume,
			};
		}
		...
		function ReadAloudAction({ messageId, sessionId, t, sessions }) {
			...
			// Only the message actually being read gets pause/resume + stop;
			// other messages keep a plain play button (paused is global).
			const active = speaking;
			// Idle: play. Speaking: pause. Paused: resume.
			const mainLabel = speaking ? t(paused ? "resume" : "pause") : t("read");
			const MainIcon = speaking ? (paused ? IconPlay : IconPause) : IconPlay;
			const mainAction = () => {
				if (!speaking) speech.speak(messageId, text);
				...
				react.createElement(
					...
					react.createElement(
						"button",
						{
							...
							"aria-label": speaking
								? t(paused ? "resume.aria" : "pause.aria")
								: t("read.aria"),
							"aria-pressed": speaking || undefined,
							onClick: mainAction
						},
						react.createElement(MainIcon, { size: 16 })
					)
				),
				active
					? react.createElement(
						Tooltip,
						{ label: t("stop"), side: "bottom" },
						react.createElement(
							"button",
							{
								type: "button",
							...
							react.createElement(IconStop, { size: 16 })
						)
					)
					: null
			);
		...
		function ReadAllAction({ sessionId, t, sessions }) {
			...
			const label = !reading
				? t("readAll")
				: t(paused ? "readAll.resume" : "readAll.pause");
			const Icon = !reading ? IconPlay : paused ? IconPlay : IconPause;
			const mainAction = () => {
				if (!reading) {
			...
			return react.createElement(
				...
				active
					? react.createElement(
						Tooltip,
						{ label: t("readAll.stop"), side: "bottom" },
						react.createElement(
							"button",
							{
								type: "button",
								"data-read-aloud-all": true,
								"aria-label": t("readAll.stop"),
								onClick: () => speech.cancel()
							},
		...
		function apply(ctx) {
			...
			// Keyed slot: `key` is the settings namespace this card edits. The
			// host half registers the same namespace, which is what makes the
			// Plugins tab dispatch this card (a key the Host does not serve is
			// skipped).
