...
/**
 * Settings namespace this plugin owns. The browser half registers its
 * Settings-page card into the keyed slot `settings.plugin.item` under this
 * same key; the Plugins tab only dispatches keys the Host serves, so this
 * registration is what makes the card appear.
 */
export const READ_ALOUD_NAMESPACE = settingsNamespace("dsh-read-aloud");

/**
 * Host-side mirror of the settings the browser card edits. The card persists
 * them in localStorage (browser-local preferences); this schema only declares
 * the namespace's shape so the Host can serve it to the settings tab.
 */
const ReadAloudSettingsSchema = z.object({
  backend: z.union(["browser", "system"]).default("browser"),
...
export function apply(ctx) {
  // Serve the settings namespace (when a settings provider is mounted) so the
  // Plugins tab dispatches the browser half's card registered under this key.
  ctx.inject(["settings"], (settingsCtx) => {
    settingsCtx.settings.register(READ_ALOUD_NAMESPACE, ReadAloudSettingsSchema);
  });
