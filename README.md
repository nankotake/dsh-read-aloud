...
- A **session-header "Read conversation" button** that reads every user message and each turn's final assistant reply in order, toggling to "Stop reading" while playing, plus a **settings gear** for the speech rate and voice.
...
- Click the **gear** beside the "Read conversation" button to open the settings popover and pick the speech rate (0.5×–2×) and the system voice.
...
- Both choices persist across page reloads in `localStorage`.
